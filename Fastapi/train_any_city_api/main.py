...
def save_model_in_django(city, model, scaler,token):
    try:

        model_io = io.BytesIO()
        pickle.dump(model, model_io)
        model_binary = model_io.getvalue()

        scaler_io = io.BytesIO()
        pickle.dump(scaler, scaler_io)
        scaler_binary = scaler_io.getvalue()

        headers = {"Authorization": token}

        response = requests.post("http://127.0.0.1:8000/api/user/save_model/", json={
            "city": city,
            "model_data": list(model_binary),
            "scaler_data": list(scaler_binary)
        },headers=headers)

        return response.json()
    
    except requests.exceptions.RequestException as e:
        return {"error": f"Failed to communicate with Django: {str(e)}"}
...
def load_model_from_django(city,token):
    try:
        headers = {"Authorization": token}
        response = requests.get(f"http://127.0.0.1:8000/api/user/get_model/{city}/",headers=headers)
        
        if response.status_code == 200:
            data = response.json()

            if "model_data" not in data or "scaler_data" not in data:
                return None, None
            
            model_binary = bytes(data["model_data"])
            scaler_binary = bytes(data["scaler_data"])

            model_io = io.BytesIO(model_binary)
            model = pickle.load(model_io)

            scaler_io = io.BytesIO(scaler_binary)
            scaler = pickle.load(scaler_io)
            ...
            return {"error": f"Failed to retrieve model, status code: {response.status_code}"}
...
@app.get("/train/{city}/{years}")
def predict_weather(city: str, years: int,request: Request):
    try:
        ...
        save=save_model_in_django(city, model, scaler,token)
...
def predict_weather(city: str,request: Request):
    try:
        token = request.headers.get("Authorization")
        if not token:
            raise HTTPException(status_code=401, detail="Missing Authorization Token")
        
        model, scaler=load_model_from_django(city,token)

        if model is None or scaler is None:
            return {"status": "faild,modul does not exist"}
